...
namespace CalendarBot.Dialogs
{
    public class MainDialog : LogoutDialog
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var userCard = _adaptiveCardService.GetUserCard(user, userPhoto);
            ...
            CancellationToken cancellationToken)
        {
            ...
            // Use defaults if time properties aren't set
            // in user's mailbox settings
            var preferredTimeZone = string.IsNullOrEmpty(user.MailboxSettings?.TimeZone) ?
            ...
            var dateTimeFormat = string.IsNullOrEmpty(dateFormat) || string.IsNullOrEmpty(timeFormat) ?
                "G" : $"{dateFormat} {timeFormat}";

            var userTimeZone = TZConvert.GetTimeZoneInfo(preferredTimeZone);
            ...
            {
                var eventCard = _adaptiveCardService.GetEventCard(calendarEvent, dateTimeFormat);
